...
/*   Updated: 2023/12/22 11:50:29 by jberay           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	// 	sort_big_a(&a_head, &b_head, size - 1);
	// }
	else
	{
	...
	while (a_head != NULL)
	{
		...
		printf("A - %d | out - %d | A - %d | index - %d\n", a_head->element, a_head->final_pos, a_head->element, a_head->index);
		a_head = a_head->next;
	}
	while (b_head != NULL)
	{
		printf("B - %d | out - %d | B - %d | index - %d\n", b_head->element, b_head->final_pos, b_head->element, b_head->index);
		b_head = b_head->next;
	}
	ps_lstclear(&a_head);
}
